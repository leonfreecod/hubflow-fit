...
export function SchedulePage() {
  ...
  const [deleteCandidate, setDeleteCandidate] = useState<ScheduleEvent | null>(null);
  ...
  async function cancelEvent(event: ScheduleEvent) {
    setActiveActionId(event.id);
    try {
      await collection.mutate(() => repositories.schedule.cancel(event.id));
    } catch {
      // The collection displays the normalized error above the timeline.
    } finally {
      setActiveActionId(null);
  ...
  const calendarLabel = new Intl.DateTimeFormat('pt-BR', { month: 'short' })
    .format(today)
    .replace('.', '')
    .toUpperCase();
  ...
  return (
    <div className="page-stack">
      <header className="page-heading">
        <div>
          <span className="eyebrow">ORGANIZAÇÃO DA ROTINA</span>
          <h1>Agenda</h1>
          <p>Gerencie avaliações, sessões individuais, treinos em grupo e encontros online.</p>
        </div>
        <Button
          icon={<Plus size={18} />}
          onClick={openCreate}
      ...
      <section className="agenda-layout">
        <Card className="calendar-summary">
          <div className="calendar-summary__month">
            <span>{calendarLabel}</span>
            <strong>{calendarYear}</strong>
          </div>
          <div className="mini-calendar">
            {['D', 'S', 'T', 'Q', 'Q', 'S', 'S'].map((day, index) => (
              <span key={`${day}${index}`} className="mini-calendar__weekday">
                {day}
              </span>
            ))}
            ...
            {Array.from({ length: daysInMonth }, (_, index) => index + 1).map((day) => (
              <button
                ...
                key={day}
                className={
                  day === today.getDate() ? 'today' : eventDays.has(day) ? 'has-event' : ''
                }
              >
                {day}
              </button>
          ...
          <div className="calendar-legend">
            <span>
              <i className="dot dot--yellow" /> Com agenda
            </span>
            <span>
              <i className="dot dot--outline" /> Hoje
            </span>
          </div>
        </Card>
        <Card>
          <div className="section-heading">
            ...
            <span className="soft-pill">
              {sorted.filter((item) => item.status === 'SCHEDULED').length} agendados
            </span>
          </div>
          <div className="timeline">
            {collection.loading ? (
              <EmptyState
                ...
                description="Aguarde enquanto os eventos são consultados."
              />
            ) : sorted.length === 0 ? (
              <EmptyState
                title="Nenhum evento encontrado"
                description="Crie um horário para começar a organizar a agenda."
              />
            ) : (
              sorted.map((event) => (
                <article className="timeline-item" key={event.id}>
                  ...
                  <div className="timeline-item__card">
                    ...
                    <div className="event-meta">
                      <span>
                        <Clock3 size={15} /> {event.durationMinutes} min
                      </span>
                      <span>
      ...
      >
        <form onSubmit={submit} className="form-grid">
          ...
          {!editing && (
            <label className="field">
              ...
              <input
                ...
                onChange={(e) => setForm({ ...form, recurrenceWeeks: Number(e.target.value) })}
              />
            </label>
          )}
          <label className="field field--span-2">
          ...
          <div className="modal-actions field--span-2">
            ...
            <Button type="submit" icon={<CalendarPlus size={18} />} disabled={submitting}>
              {submitting ? 'Salvando...' : editing ? 'Salvar alterações' : 'Agendar'}
            </Button>
          </div>
        </form>
      </Modal>
      <ConfirmDialog
        ...
        description={
          deleteCandidate
            ? `${deleteCandidate.title}, de ${deleteCandidate.studentName}, será removido da agenda.`
            : ''
        }
        ...
        onConfirm={() => {
          if (deleteCandidate) void removeEvent(deleteCandidate);
        }}
      />
    </div>
  );
