...
export function StudentsPage() {
  ...
  return (
    <div className="page-stack">
      ...
      {collection.error && !modalOpen && <div className="form-error" role="alert">{collection.error}</div>}
      ...
      <Card>
        <div className="table-wrap">
          {collection.loading
            ? <EmptyState title="Carregando alunos" description="Aguarde enquanto os dados são consultados." />
            ...
            <table className="data-table"><thead><tr><th>Aluno</th><th>Plano</th><th>Mensalidade</th><th>Próxima cobrança</th><th>Status</th><th aria-label="Ações" /></tr></thead><tbody>
              {filtered.map((student) => (
                <tr key={student.id}>
                  ...
                  <td><div className="row-actions"><button onClick={() => openEdit(student)} aria-label="Editar"><Pencil size={17} /></button><button onClick={() => void removeStudent(student)} aria-label="Excluir" disabled={deletingId === student.id}><Trash2 size={17} /></button></div></td>
                </tr>
              ))}
            </tbody></table>
          )}
      ...
      >
        <form onSubmit={submit} className="form-grid">
          {collection.error && <div className="form-error field--span-2" role="alert">{collection.error}</div>}
          ...
          <div className="modal-actions field--span-2"><Button type="button" variant="secondary" onClick={() => setModalOpen(false)}>Cancelar</Button><Button type="submit" icon={<UserRoundCheck size={18} />} disabled={submitting}>{submitting ? 'Salvando...' : editing ? 'Salvar alterações' : 'Cadastrar aluno'}</Button></div>
        </form>
      </Modal>
    </div>
  );
