...
export function FinancePage() {
  ...
  async function submit(event: React.FormEvent) {
    ...
    const paidAt = form.status === 'PAID'
      ? editing?.paidAt ?? currentDate()
      : undefined;
  ...
  async function removePayment(payment: Payment) {
    if (!confirm(`Excluir a cobrança "${payment.description}" de ${payment.studentName}?`)) return;
    setActiveActionId(payment.id);
    try {
  ...
  return (
    <div className="page-stack">
      <header className="page-heading"><div><span className="eyebrow">CONTROLE FINANCEIRO</span><h1>Financeiro</h1><p>Acompanhe mensalidades, pendências e recebimentos sem depender de planilhas.</p></div><Button icon={<Plus size={18} />} onClick={openCreate} disabled={studentsLoading || students.length === 0}>Nova cobrança</Button></header>
      ...
      <Card>
        ...
        <div className="table-wrap">
          {collection.loading
            ? <EmptyState title="Carregando pagamentos" description="Aguarde enquanto as cobranças são consultadas." />
            : filtered.length === 0
              ? <EmptyState title="Nenhum pagamento encontrado" description="Ajuste o filtro ou crie uma nova cobrança." />
              : <table className="data-table"><thead><tr><th>Aluno</th><th>Descrição</th><th>Vencimento</th><th>Método</th><th>Valor</th><th>Status</th><th>Ações</th></tr></thead><tbody>
                {filtered.map((payment) => <tr key={payment.id}><td><strong>{payment.studentName}</strong></td><td>{payment.description}</td><td>{formatDate(payment.dueDate)}</td><td>{payment.method}</td><td><strong>{formatCurrency(payment.amount)}</strong></td><td><Badge status={payment.status} /></td><td><div className="row-actions">{payment.status !== 'PAID' && <button className="table-action" onClick={() => void markPaid(payment)} disabled={activeActionId === payment.id}><CircleDollarSign size={17} /> Baixar</button>}<button onClick={() => openEdit(payment)} aria-label="Editar pagamento"><Pencil size={17} /></button><button onClick={() => void removePayment(payment)} aria-label="Excluir pagamento" disabled={activeActionId === payment.id}><Trash2 size={17} /></button></div></td></tr>)}
              </tbody></table>}
        </div>
      </Card>
      ...
      >
        <form onSubmit={submit} className="form-grid">
          ...
          <label className="field field--span-2"><span>Aluno</span><select required value={form.studentId} onChange={(e) => setForm({ ...form, studentId: e.target.value })}>{students.map((student) => <option value={student.id} key={student.id}>{student.name}</option>)}</select></label>
          ...
          <label className="field"><span>Valor</span><input type="number" min="0" step="0.01" required value={form.amount} onChange={(e) => setForm({ ...form, amount: Number(e.target.value) })} /></label>
          <label className="field"><span>Vencimento</span><input type="date" required value={form.dueDate} onChange={(e) => setForm({ ...form, dueDate: e.target.value })} /></label>
          ...
          <div className="modal-actions field--span-2"><Button type="button" variant="secondary" onClick={() => setModalOpen(false)}>Cancelar</Button><Button type="submit" icon={<CircleDollarSign size={18} />} disabled={submitting}>{submitting ? 'Salvando...' : editing ? 'Salvar alterações' : 'Criar cobrança'}</Button></div>
        </form>
      </Modal>
    </div>
  );
