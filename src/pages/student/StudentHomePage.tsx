...
export function StudentHomePage() {
  ...
  const nextEvent = useMemo(() => events.find((item) => item.status === 'SCHEDULED'), [events]);
  const pendingPayment = payments.find((item) => item.status !== 'PAID');

  return (
    <div className="page-stack">
      <header className="student-hero">
        <div><span className="eyebrow">MINHA JORNADA</span><h1>Vamos em frente, {getFirstName(user?.name ?? 'Atleta')}.</h1><p>Consistência hoje. Resultado amanhã.</p></div>
        <div className="student-hero__goal"><span><Target size={20} /></span><div><small>OBJETIVO ATUAL</small><strong>{student?.goal ?? 'Carregando...'}</strong></div><b>{student?.progress ?? 0}%</b></div>
      </header>

      <section className="stats-grid">
        <StatCard label="Consistência" value="88%" helper="últimos 30 dias" trend={9.2} icon={Flame} />
        <StatCard label="Treinos concluídos" value="14" helper="neste mês" trend={16.6} icon={CheckCircle2} />
        <StatCard label="Programas ativos" value={String(plans.length)} helper="disponíveis agora" icon={Dumbbell} />
        <StatCard label="Mensalidade" value={pendingPayment ? formatCurrency(pendingPayment.amount) : 'Em dia'} helper={pendingPayment ? `vence ${formatDate(pendingPayment.dueDate)}` : 'nenhuma pendência'} icon={CreditCard} />
      </section>
      ...
      <section className="dashboard-grid">
        <Card><div className="section-heading"><div><span>MEUS PROGRAMAS</span><h2>Planos ativos</h2></div></div><div className="student-plan-list">{plans.map((plan) => <div key={plan.id}><span><Dumbbell size={20}/></span><div><strong>{plan.name}</strong><p>{plan.description}</p><small>{plan.weeks} semanas · {plan.sessionsPerWeek} sessões/semana</small></div><Badge status={plan.level}/></div>)}</div></Card>
        <Card><div className="section-heading"><div><span>FINANCEIRO</span><h2>Últimos pagamentos</h2></div></div><div className="payment-cards">{payments.slice(0,3).map((payment) => <div key={payment.id}><div><strong>{payment.description}</strong><span>{formatDate(payment.dueDate)}</span></div><div><strong>{formatCurrency(payment.amount)}</strong><Badge status={payment.status}/></div></div>)}</div></Card>
      </section>
    </div>
  );
}
